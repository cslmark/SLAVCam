...
- (id)init {
    ...
    if (self) {
        captureQueue = dispatch_get_main_queue();
...
- (void)replacePixelData:(CMSampleBufferRef)sampleBuffer{
    CVPixelBufferRef pixelBufferOrigin = CMSampleBufferGetImageBuffer(sampleBuffer);
    CVPixelBufferRef pixelBufferNew = currentSampleBuffer;
    
    int bufferHeight = (int)CVPixelBufferGetHeight( pixelBufferOrigin );
    size_t bytesPerRow = CVPixelBufferGetBytesPerRow( pixelBufferOrigin );
    
    CVPixelBufferLockBaseAddress( pixelBufferOrigin, 0 );
    CVPixelBufferLockBaseAddress( pixelBufferNew, 0 );
    uint8_t *dst = CVPixelBufferGetBaseAddress( pixelBufferOrigin );
    uint8_t *src = CVPixelBufferGetBaseAddress( pixelBufferNew );
    
    memcpy(dst, src, bytesPerRow*bufferHeight);
    
    CVPixelBufferUnlockBaseAddress( pixelBufferNew, 0 );
    CVPixelBufferUnlockBaseAddress( pixelBufferOrigin, 0 );
}

- (void)setVideoSampleBuffer:(CVPixelBufferRef)sampleBuffer {
